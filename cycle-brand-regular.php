...
// Buscar todos os produtos da categoria 'Cycle Brand Regular Square'
$queryCycleBrand = "SELECT * FROM produtos 
                   WHERE categoria = 'Cycle Brand Regular Square' 
                   AND tipo = 'produto'
                   ORDER BY nome";
$stmtCycleBrand = $conn->prepare($queryCycleBrand);
$stmtCycleBrand->execute();
$cycleBrandProducts = $stmtCycleBrand->fetchAll(PDO::FETCH_ASSOC);
...
<head>
    ...
    <title>Cycle Brand Regular Square - Flute Incensos</title>
    ...
    <script>
        ...
        // Favoritos (mesmo comportamento da index.php)
    ...
    <div class="main-container">
        <main class="products-area">
            <div class="main-text2">
                <h2>Cycle Brand Regular Square</h2>

                
                <div class="products-grid">
                    <?php foreach ($cycleBrandProducts as $produto): 
                        // Título padronizado
                        $nomePadronizado = formatarTituloProduto($produto['categoria'], $produto['nome']);
                        // Variação para imagem (como na index)
                        $base = $produto['nome'];
...
<style>
  /* Alinhar layout dos cards como na index.php */
  ...
  .product-info { text-align: left; display: flex; flex-direction: column; align-items: flex-start; width: 100%; }
  .price-area { width: 100%; }
  .cart-controls { width: 100%; display: flex; gap: 10px; align-items: center; flex-wrap: wrap; flex-direction: column; align-items: flex-start; }
  ...
  /* Ícone de favoritos (coração em círculo) */
...
<script>
  ...
  (function(){
    function clampToInt(val){ var n = parseInt(val, 10); if (isNaN(n) || n < 1) n = 1; return n; }
