<?php
// Página de Produto: Caixa Incenso Flute Cycle Brand Regular - Boa Sorte
session_start();
...
// Busca o produto específico por nome/categoria (teste)
$query = "SELECT * FROM produtos 
          WHERE tipo = 'produto' 
          AND (categoria = 'Cycle Brand Regular Square' OR categoria = 'Cycle Brand Regular')
          AND nome LIKE :nome
          ORDER BY id DESC LIMIT 1";
$stmt = $conn->prepare($query);
$stmt->execute([':nome' => '%Boa Sorte%']);
$produto = $stmt->fetch(PDO::FETCH_ASSOC);

if (!$produto) {
    // fallback: cria dados mínimos para não quebrar
    $produto = [
        'id' => 0,
        'nome' => 'Cycle Brand Regular - Boa Sorte',
        'categoria' => 'Cycle Brand Regular Square',
        'preco' => 0,
    ];
}

// Título padronizado e variação para imagem
...
$relStmt = $conn->prepare("SELECT * FROM produtos WHERE tipo='produto' AND categoria = :cat AND id <> :id ORDER BY RAND() LIMIT 4");
$relStmt->execute([
...
<head>
    ...
    <style>
      ...
      .pp-actions { display: flex; flex-direction: column; gap: 10px; max-width: 360px; }
      .pp-qty { display:flex; align-items:center; gap:10px; }
      .pp-qty input { width: 80px; padding: 8px; }
      .btn-primary { background:#0f6; color:#fff; padding:12px 16px; border-radius:6px; border:none; cursor:pointer; font-weight:600; }
      .btn-wpp { border:1px solid #25d366; color:#25d366; padding:10px 16px; border-radius:6px; text-align:center; display:inline-block; }
...
<body>
    ...
    <main class="product-page">
        ...
        <section class="pp-desc">
            <h3>Descrição</h3>
            <p><strong><?php echo htmlspecialchars($nomePadronizado); ?></strong> faz parte da linha Cycle Brand Regular, ideal para o dia a dia. Aroma clássico para harmonizar ambientes e elevar o bem-estar.</p>
        </section>
        ...
        <section class="pp-specs">
            ...
            <div class="box">
                ...
                <p><strong>QUANTIDADE:</strong> 1 caixinha (conteúdo padrão Cycle Brand Regular)</p>
