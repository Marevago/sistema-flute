...
// Buscar todos os produtos da categoria 'Incenso Xamânico Tube'
$queryXamanico = "SELECT * FROM produtos 
                   WHERE categoria = 'Incenso Xamânico Tube' 
                   AND tipo = 'produto'
                   ORDER BY nome";
$stmtXamanico = $conn->prepare($queryXamanico);
$stmtXamanico->execute();
$xamanicoProducts = $stmtXamanico->fetchAll(PDO::FETCH_ASSOC);
...
<head>
    ...
    <title>Xamânico Tube - Flute Incensos</title>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // Inicializações globais (fora de handlers)
        ...
        // Favoritos (escopo global, igual à index.php)
        async function inicializarFavoritos(){
            try {
                ...
                document.querySelectorAll('.favorite-icon[data-produto-id]').forEach(el=>{
                    ...
                    el.addEventListener('click', (e)=>{ e.preventDefault(); e.stopPropagation(); toggleFavorito(el, id); });
                });
            } catch(e) {}
        }
        async function toggleFavorito(el, produtoId){
            ...
            try {
                const resp = await fetch('favoritos.php', { method:'POST', credentials: 'same-origin', headers:{'Content-Type':'application/json'}, body: JSON.stringify({ action: adding ? 'add' : 'remove', produto_id: produtoId }) });
...
<body>
    ...
    <div class="main-container">
        <main class="products-area">
            <div class="main-text2">
                <h2>Incensos Xamânico Tube</h2>

                
                <div class="products-grid">
                    <?php foreach ($xamanicoProducts as $produto): 
    ...
    <style>
      /* Deixar os cards idênticos aos da index.php */
    ...
    <script>
      // Comportamento do stepper de quantidade (igual index.php)
      (function(){
