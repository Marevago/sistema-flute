...
// Buscar todos os produtos da categoria 'Cycle Brand Rectangle'
$queryCycleBrandRectangle = "SELECT * FROM produtos 
                   WHERE categoria = 'Cycle Brand Rectangle' 
                   AND tipo = 'produto'
                   ORDER BY nome";
$stmtCycleBrandRectangle = $conn->prepare($queryCycleBrandRectangle);
$stmtCycleBrandRectangle->execute();
$cycleBrandRectangleProducts = $stmtCycleBrandRectangle->fetchAll(PDO::FETCH_ASSOC);
...
<head>
    ...
    <style>
        ...
        /* Stepper de quantidade (igual ao index) */
        ...
        /* Botões largura total iguais ao index */
        .btn-full { display: block; width: 100%; box-sizing: border-box; }

        /* Favorite icon visuals aligned with index */
    ...
    <script>
        ...
        // Favoritos (igual às demais páginas)
...
<body>
    ...
    <div class="main-container">
        <main class="products-area">
            <div class="main-text2">
                <h2>Cycle Brand Rectangle</h2>

                
                <div class="products-grid">
                    <?php foreach ($cycleBrandRectangleProducts as $produto): 
                        // 1. Preparar nome padronizado para EXIBIÇÃO
    ...
    <script>
        ...
        // Atualiza o contador, inicializa favoritos e stepper quando a página carrega
        document.addEventListener('DOMContentLoaded', function(){
            atualizarContadorCarrinho();
            inicializarFavoritos();
            // Comportamento do stepper de quantidade
            function clampToInt(val){ var n = parseInt(val, 10); return isNaN(n) || n < 1 ? 1 : n; }
            document.querySelectorAll('.qty-stepper').forEach(function(step){
                ...
                if (!input) return;
                var minus = step.querySelector('.qty-btn.minus');
                var plus = step.querySelector('.qty-btn.plus');
                minus && minus.addEventListener('click', function(){ input.value = Math.max(1, clampToInt(input.value) - 1); });
                plus && plus.addEventListener('click', function(){ input.value = clampToInt(input.value) + 1; });
                input.addEventListener('input', function(){ input.value = clampToInt(input.value); });
            });
        });
    </script>
</body>
